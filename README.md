...
[Play in Browser Itch.io](https://ckendrick-mqu.itch.io/counter-knights-arcane-offensive) 

![Counter Knights Gameplay](./screenshots/knights.png)

A Unity WebGL application demonstrating real-time vector maths, deterministic state management, and algorithmic input calculation. While presented as a tactical 2D shooter, the project serves as a practical application of custom physics mathematics, AI state machines, and browser-based performance optimisation.

---

## Core Application Loop

* **Objective:** Players must navigate a 2D environment and eliminate all target entities within a strict time constraint.
...
* **Algorithmic Recoil:** The primary player challenge revolves around a dynamic recoil system. Player movement and rapid inputs continuously increase a deviation variable, forcing players to balance input speed with calculated accuracy. Safe zones within the environment naturally pace the application, allowing decay rates to reset.
...
* **Mathematical Deviation Tracking:** The core combat loop relies on a custom recoil algorithm. It calculates precise deviation using dynamic variables that increase with action frequency and decay over time. This ensures players input directly dictates the underlying mathematical state.
* **Finite State Machines (FSM):** Enemy logic is driven by raycasting for line-of-sight checks. Establishing or breaking LOS triggers a lightweight FSM (Idle → Detect → Execute) that manages AI behaviour without overloading the main execution thread.
